...
animate_slide_down() {
  ...
  hypr_move_window_pixel "$target_x" "$start_y" "address:$addr" >/dev/null 2>&1
  sleep 0.05

  # Animate slide down
  for i in $(seq 1 $SLIDE_STEPS); do
    local current_y=$((start_y + (step_y * i)))
    hypr_move_window_pixel "$target_x" "$current_y" "address:$addr" >/dev/null 2>&1
    sleep 0.03
  done

  # Ensure final position is exact
  hypr_move_window_pixel "$target_x" "$target_y" "address:$addr" >/dev/null 2>&1
}
...
animate_slide_up() {
  ...
  for i in $(seq 1 $SLIDE_STEPS); do
    local current_y=$((start_y - (step_y * i)))
    hypr_move_window_pixel "$start_x" "$current_y" "address:$addr" >/dev/null 2>&1
    sleep 0.03
  done
...
spawn_terminal() {
  ...
  hypr_exec_cmd "$TERMINAL_CMD" "{ float = true, size = { $width, $height }, workspace = \"special:scratchpad silent\" }"
  ...
  if [ -n "$new_addr" ] && [ "$new_addr" != "null" ]; then
    ...
    hypr_move_window "$CURRENT_WS" "address:$new_addr"
    hypr_pin_window "address:$new_addr"
...
if terminal_exists; then
  ...
  if [ "$focused_monitor" != "$dropdown_monitor" ]; then
    ...
    hypr_move_window_pixel "$target_x" "$target_y" "address:$TERMINAL_ADDR"
    hypr_resize_window_pixel "$width" "$height" "address:$TERMINAL_ADDR"
    # Update ADDR_FILE
  ...
  if terminal_in_special; then
    ...
    # Use movetoworkspacesilent to avoid affecting workspace history
    hypr_move_window "$CURRENT_WS" "address:$TERMINAL_ADDR"
    hypr_pin_window "address:$TERMINAL_ADDR"

    # Set size and animate slide down
    hypr_resize_window_pixel "$width" "$height" "address:$TERMINAL_ADDR"
    animate_slide_down "$TERMINAL_ADDR" "$target_x" "$target_y" "$width" "$height"

    hypr_focus_window "address:$TERMINAL_ADDR"
  else
    ...
    if [ -n "$geometry" ]; then
      ...
      sleep 0.1
      hypr_pin_window "address:$TERMINAL_ADDR" # Unpin (toggle)
      hypr_move_window "$SPECIAL_WS" "address:$TERMINAL_ADDR"
    else
